...
echo "Starting Strong Scaling..."
	...
	for matrix in "${MATRICES[@]}"; do
		for procs in 1 2 4 8 16 32 64 128; do
			...
			mpirun -np $procs -bind-to core numactl --interleave=all ./spmv_mpi_benchmark "$matrix" "-ss"
		done
	done
...
for procs in 1 2 4 8 16 32 64 128; do
	...
	if [ -f "$WEAK_MATRIX" ]; then
		...
		mpirun -np $procs -bind-to core numactl --interleave=all ./spmv_mpi_benchmark "$WEAK_MATRIX" "-ws"
	else
